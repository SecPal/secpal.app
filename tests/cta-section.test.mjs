...
test("CTA uses semantic contact markup and contact-first action hierarchy", () => {
  ...
  assert.doesNotMatch(
    component,
    /SecPal soll sich im Arbeitsalltag bewähren|SecPal should work in day-to-day operations|Kontakt aufnehmen|Get in touch|Entwicklung auf GitHub ansehen|View development on GitHub/
  );
});
...
test("desktop and mobile navigation point to the localized contact section", () => {
  ...
  assert.equal(nav.match(/href="https:\/\/github\.com\/SecPal"/g)?.length, 4);
  assert.equal(nav.match(/\{t\.nav\.github\}/g)?.length, 2);
  assert.equal(nav.match(/\{t\.nav\.followProgress\}/g)?.length, 2);
